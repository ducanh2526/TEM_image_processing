...
from tensorflow.keras.layers import Layer, Flatten, Dense, Lambda
from tensorflow.keras import backend
from tensorflow.keras.applications.mobilenet_v2 import MobileNetV2

FILTERS = [512, 512, 512, 512, 256, 128, 64, 32]
# FILTERS = [64, 64, 64, 64, 64, 32, 16, 16]
...
class PGAN(Model):
    ...
    def init_encoder(self):
        img = layers.Input(shape=(self.last_shape, self.last_shape, 3))

        dense = MobileNetV2(include_top=False, input_shape=(
            self.last_shape, self.last_shape, 3), weights='imagenet', pooling='avg')

        x = dense(img)

        x = Flatten()(x)
        h = Dense(self.latent_dim*2, activation='relu')(x)

        z_mean = Dense(self.latent_dim)(h)
        z_log_var = Dense(self.latent_dim, activation='tanh')(h)
        z = Lambda(self.sampler, output_shape=(
            self.latent_dim,))([z_mean, z_log_var])

        encoder = Model(img, [z, z_mean, z_log_var], name='Encoder')
        encoder.summary()
        return encoder
    ...
    def stabilize_generator(self):
        ...
        # self.generator.summary()
    def stabilize_encoder(self,e_optimizer):
    ...
    def train_step_2(self, real_images):
        ...
        return {'d_loss': d_loss, 'g_loss': g_loss, 'drift': drift, 'gp': gp}
    ...
    def train_step(self, real_images):
        ...
        for i in range(self.d_steps):
            ...
            if self.encoder:
                with tf.GradientTape() as tape:
                    #Get the encoded latents z
                    real_images_3 = tf.tile(real_images, [1, 1, 1, 3])
                    encoded_z, z_mean, z_log_var = self.encoder(real_images_3)

                    #Recover image
                    recover_image = self.generator(encoded_z, training=True)

                    # Get the logits for the recover images
                    recover_logits = self.discriminator(recover_image, training=True)

                    recover_cost = tf.reduce_mean(recover_logits) - tf.reduce_mean(real_logits)

                    pixel_cost = tf.reduce_mean(tf.square(recover_image - real_images))
                    
                    #Piority for gaussian
                    L_prior = - 0.5 * tf.reduce_sum(1. + tf.clip_by_value(z_log_var, -2, 2) - tf.square(z_mean) -
                                                tf.exp(tf.clip_by_value(z_log_var, -2, 2)), axis=-1) / self.last_shape / self.last_shape + 1e-8

                    e_loss = recover_cost + pixel_cost + L_prior


                e_gradient = tape.gradient(e_loss, self.encoder.trainable_variables)
                # Update the weights of the discriminator using the discriminator optimizer
                self.e_optimizer.apply_gradients(zip(e_gradient, self.encoder.trainable_variables))
        ...
        self.g_optimizer.apply_gradients(zip(g_gradient, self.generator.trainable_variables))
        if self.encoder:
            return {'d_loss': d_loss, 'g_loss': g_loss, 'drift': drift, 'gp': gp, 'e_loss': e_loss}
        else:
            return {'d_loss': d_loss, 'g_loss': g_loss, 'drift': drift, 'gp': gp}